...
let handler = async (m, { conn, args, isAdmin, isOwner }) => {
    ...
    if (args[0] === "on") {
        if (antiTagSWGroup[m.chat]) return m.reply("😡 Udah nyala dari tadi, ngapain dinyalain lagi?!")
        antiTagSWGroup[m.chat] = true
        saveAntiTagSW(antiTagSWGroup)
        return m.reply("✅ *Anti Tag Status WhatsApp* sekarang AKTIF di grup ini!")
    } else if (args[0] === "off") {
        if (!antiTagSWGroup[m.chat]) return m.reply("😔 Udah mati dari tadi, lu baru sadar?")
